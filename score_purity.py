    ...
    with open(file_name) as tsv_file:
        for line in tsv_file:
            if line[0] == "h":
                ...
                haplotype = data[0]
                contig = data[1]
                matched_total_percent = data[3]
                delta_homoeolog = data[12]
                delta_homolog = data[20]
...
if __name__ == "__main__":
    ...
    impurity, explained_by, sum_percentage = score(purity)
    print("IMPURITY: {0} DELTA: {1} ({2})".format(impurity, explained_by, sum_percentage))
